...
class CLIController2:
    # version   显示当前软件的版本
    def __init__(self):
        self.PROCESS = None

    def start_process(self, nodepath):
        print("start process: " + nodepath)
    ...
    def check_process(self):
        ...
        # self.PROCESS.stdout.readlines(100)

    def write_process(self, cmd):
        print("cli cmd: " + cmd)
        self.PROCESS.stdin.write((cmd + "\n").encode())
        print("cli cmd end: " + cmd)

    def read_process(self, timeout=3):
        clireadthread = CLIReadThread(self.PROCESS)
        clireadthread.start()
        waitsec = 0
        linestr = ""
        while True:
            lines = clireadthread.readlines()
            if lines is None:
                if waitsec >= 3:
                ...
                for line in lines:
                    linestr += line.decode('unicode-escape').encode('utf-8')
        return linestr
        ...
        waitsec = 0
        while True:
            linebytes = self.PROCESS.stdout.readline()
            if linebytes is None:
                if waitsec >= 3:
                    raise "read neo-cli process timeout: " + timeout
                time.sleep(1)
                waitsec = waitsec + 1
            else:
                linestr = linebytes.decode('unicode-escape').encode('utf-8')
        print(linestr)
        return linestr
        '''
